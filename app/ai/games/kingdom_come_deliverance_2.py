...
PC_CONFIGS = [
    ...
    {
        "quality": "ultra",
        "cpu": "Процессор AMD Ryzen 9 5900XT BOX",
        "gpu": "Видеокарта MSI GeForce RTX 5060 Ti GAMING TRIO OC [RTX 5060 Ti 8G GAMING TRIO OC]",
        "dimm": "Оперативная память Apacer NOX [AH4U32G26C08YMBAA-2] 32 ГБ",
...
def create_game_table():
    try:
        ...
        # Создаем таблицу games.kingdom_come_deliverance_2
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS games.kingdom_come_deliverance_2 (
        ...
        for config in PC_CONFIGS:
            cursor.execute(
                sql.SQL("""
                    INSERT INTO games.kingdom_come_deliverance_2 (quality, cpu, gpu, ssd_m2, dimm)
                    VALUES (%s, %s, %s, %s, %s)
        ...
        # Сохраняем изменения
        conn.commit()
        print("Таблица 'games.kingdom_come_deliverance_2' успешно создана и заполнена!")
